...
vector<FuncInfo> funcDeclaration; // {函数名, 首地址}
...
bool MainWindow::assign_statement(State src, State dst)
{
    if (std::find(declarations.begin(), declarations.end(), dst.place) == declarations.end())
    {
        ...
        error_message+=": " + dst.place + " none definition\n";
...
bool func_statement(State type, State M, State idt)
{
    FuncInfo func;
    func.name = idt.place;
    func.type = type.place;
    func.startAdd = M.next_quad;
    funcDeclaration.push_back(func);
    vars.clear();
    declarations.clear();
...
bool param_statement(State& fa, State expression)
{
    // emit_("par", "", "", expression.place);
    fa.par_list.push_back(expression.place);
    return true;
}

bool call_statement(State& fa, State func)
{
    emit_("call", "", "", func.place);
    emit_(":=", "@", "", "v" + to_string(vars.size()));
    fa.place = "v" + to_string(vars.size());
    vars.push_back(fa.place);
    return true;
}

bool return_void_statement()
{
    emit_("return", "", "", "");
    return true;
}

bool return_statement(State ret)
{
    emit_("return", "", "", ret.place);
    return true;
}

bool get_statement(State var)
{
    emit_("get", "", "", var.place);
    return true;
}

bool param_list_statement(State param_list)
{
    for(auto param : param_list.par_list)
    {
        emit_("par", param, "", "");
    }
...
void MainWindow::process(const vector<Token_table> &input,TextBrowserStreambuf *textBrowserStreambuf_state,TextBrowserStreambuf* textBrowserStreambuf_symbol,TextBrowserStreambuf* textBrowserStreambuf_input,TextBrowserStreambuf* textBrowserStreambuf_code,TextBrowserStreambuf* textBrowserStreambuf_obj_code,int go_end)
{
    //    qDebug()<<1;
    done = false;
    int point = 0;
            ...
            else // 规约
            {
                ...
                else
                {
                    ...
                    if (mp.count(op.second))
                    {
                        ...
                        else if (mp.at(op.second) == "func")
                        {
                            func_statement(states[n - 8], states[n - 7], states[n - 6]);
                        }
                        else if (mp.at(op.second) == "param1")
                        {
                            fa = states[n - 3];
                            param_statement(fa, states[n - 1]);
                        }
                        else if (mp.at(op.second) == "param2")
                        {
                            param_statement(fa, states[n - 1]);
                        }
                        else if(mp.at(op.second) == "param_list")
                        {
                            param_list_statement(states[n - 1]);
                        }
                        else if (mp.at(op.second) == "call")
                        {
                            call_statement(fa, states[n - 2]);
                        }
                        else if (mp.at(op.second) == "return_void")
                        {
                            return_void_statement();
                        }
                        else if (mp.at(op.second) == "return")
                        {
                            return_statement(states[n - 2]);
                        }
                        else if (mp.at(op.second) == "get")
                        {
                            get_statement(states[n - 1]);
                        }
                    }
    ...
    ObjectCodeGenerator *p = new ObjectCodeGenerator();
    p->process(obj_code, textBrowserStreambuf_obj_code);
    delete p;

    done = true;

    funcDeclaration.resize(0);
