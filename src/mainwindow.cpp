    ...
    , ui(new Ui::MainWindow)
{
    ...
    obj_code = new QTextBrowser();
    Linput = new QLabel("输入串");
    Lstate = new QLabel("语法状态栈");
    Lsymbol = new QLabel("语法符号栈");
    Lcode = new QLabel("中间代码");
    Lobjcode = new QLabel("目标代码");
    ...
    TextBrowserStreambuf textBrowserStreambuf_obj_code(obj_code);
    ...
    one_used = false;
    QObject::connect(portBtn_one,&QPushButton::clicked,[this,textBrowserStreambuf_state,textBrowserStreambuf_symbol,textBrowserStreambuf_input,textBrowserStreambuf_code,textBrowserStreambuf_obj_code]()mutable{
        if(one_used==false){
            one_used = true;
            main1(&textBrowserStreambuf_state,&textBrowserStreambuf_symbol,&textBrowserStreambuf_input,&textBrowserStreambuf_code,&textBrowserStreambuf_obj_code,false);
        }
    });
    QObject::connect(portBtn_run,&QPushButton::clicked,[this,textBrowserStreambuf_state,textBrowserStreambuf_symbol,textBrowserStreambuf_input,textBrowserStreambuf_code,textBrowserStreambuf_obj_code]()mutable{
        if(one_used==false){
            qDebug()<<"1:"<<"one_used:"<<one_used;
            main1(&textBrowserStreambuf_state,&textBrowserStreambuf_symbol,&textBrowserStreambuf_input,&textBrowserStreambuf_code,&textBrowserStreambuf_obj_code,true);
        }
    });
...
void MainWindow::dockInit(){
    ...
    portBtn_yufa->setIconSize(QSize(20,30));
    //中间代码
    ...
    portBtn_code->setIconSize(QSize(20,30));

    // 目标代码
    portBtn_obj_code = new QPushButton(this);
    portBtn_obj_code->setIcon(QIcon("./icon/obj.png"));	//按钮添加图片
    portBtn_obj_code->setToolTip("目标代码");			//鼠标移动到按钮上时显示提示
    portBtn_obj_code->setIconSize(QSize(20,30));

    btn_save_code = new QPushButton(this);
    btn_save_code->setFixedSize(QSize(30, 30));
    btn_save_code->setIcon(QIcon("./icon/save.png"));
    btn_save_code->setToolTip("保存");
    btn_save_code->setIconSize(QSize(20, 20));

    btn_save_obj_code = new QPushButton(this);
    btn_save_obj_code->setFixedSize(QSize(30, 30));
    btn_save_obj_code->setIcon(QIcon("./icon/save.png"));
    btn_save_obj_code->setToolTip("保存");
    btn_save_obj_code->setIconSize(QSize(20, 20));
    ...
    connect(portBtn_obj_code, SIGNAL(clicked()), this, SLOT(show_obj_code()));
    connect(btn_save_code, SIGNAL(clicked()), this, SLOT(save_code()));
    connect(btn_save_obj_code, SIGNAL(clicked()), this, SLOT(save_obj_code()));
}
...
void MainWindow::show_yufa()
{
    ...
    buttonLayout->addWidget(portBtn_obj_code);
    // 设置按钮和文本浏览器的垂直布局
...
void MainWindow::show_code()
{
    ...
    layout->addWidget(Lcode);
    layout->addStretch(1);
    layout->addWidget(btn_save_code);
...
void MainWindow::show_obj_code()
{
    ...
    QHBoxLayout *layout = new QHBoxLayout();
    layout->addWidget(Lobjcode);
    layout->addStretch(1);
    layout->addWidget(btn_save_obj_code);
    layout->addStretch(10);

    QVBoxLayout *layout_right = new QVBoxLayout(rightdock);
    layout_right->addLayout(buttonLayout);
    // layout_right->addWidget(Lobjcode);
    layout_right->addLayout(layout);
    layout_right->addWidget(obj_code);
    // 创建小部件并设置布局
    QWidget *widget1 = new QWidget();
    widget1->setLayout(layout_right);
    rightdock->setWidget(widget1);
...
void MainWindow::on_actionSave_obj_code_triggered()
{
    save_obj_code();
}
...
void MainWindow::save_code()
{
    if(!done)
        return;
    QFileDialog fileDialog(nullptr, "保存文件", "", "(*.txt);;所有文件 (*)");
    fileDialog.setAcceptMode(QFileDialog::AcceptSave);
