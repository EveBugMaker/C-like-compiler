...
vector<Token_table> parse(string content)
{
    ...
    while (ch = content[i++], i!=content.length()+1)
    {
        ...
        else if (isletter(ch))
        {
            ...
            {
                if (token == "else" || token == "if" || token == "while" || token == "return")
                    res.push_back({line_count, "", ""});
        ...
        else if (isdigit(ch))   //数字
        {
            ...
            while (i!=content.length() && isdigit(content[i]))
                token += content[i++];
            // δ
            if (i!=content.length() && isletter(content[i]))    //数字后面跟了字母，出错
            {
                while (i!=content.length() && (isdigit(content[i]) || isletter(content[i])))
                    token += content[i++];
                token_code = TK_UNDEF;
...
int MainWindow::main1(TextBrowserStreambuf *textBrowserStreambuf_state,TextBrowserStreambuf* textBrowserStreambuf_symbol,TextBrowserStreambuf* textBrowserStreambuf_input,TextBrowserStreambuf* textBrowserStreambuf_code,TextBrowserStreambuf* textBrowserStreambuf_obj_code,int go_end)
{
    ...
    process(parse(content),textBrowserStreambuf_state,textBrowserStreambuf_symbol,textBrowserStreambuf_input,textBrowserStreambuf_code,textBrowserStreambuf_obj_code,go_end);
    // debug();
